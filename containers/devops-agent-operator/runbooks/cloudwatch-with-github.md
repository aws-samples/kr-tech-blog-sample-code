...
## Step 6: Source Code Change Analysis (GitHub)

After analyzing container logs (`type = "container-log"`) and pod manifest (`type = "pod-manifest"`), check if the failing container image originates from a known source repository. If a related repository is identified, use GitHub to review recent code changes that may have caused the failure.

### Known Image Sources

The following images are built from the **[github-account/example-repo](https://github.com/github-account/example-repo)** repository:
...
### Image Tag to Commit SHA Mapping

These images use `sha-<commit-sha>` format as the image tag. The `****` portion of `sha-****` corresponds directly to the Git commit SHA of the source repository.

Example:
```
Image: ghcr.io/cawcaw253/sonar/web-flask:sha-a1b2c3d
                                               ^^^^^^^^
                                               Git commit SHA: a1b2c3d
```

### How to Investigate Code Changes

1. Extract the image tag from `type = "pod-manifest"` event
2. Identify the commit SHA from the `sha-****` tag
3. View the commit diff on GitHub:
   ```
   https://github.com/cawcaw253/sonar/commit/<commit-sha>
   ```
4. Compare with a previous known-good version:
   ```
   https://github.com/cawcaw253/sonar/compare/<old-sha>...<new-sha>
   ```

### When to Use This Step

- Container logs indicate an application-level error (not infrastructure)
- The failure started after a recent deployment (new image tag)
- The image matches one of the known source repositories above
- You need to understand what code changes were introduced in the failing version

---

## Step 7: When to Use kubectl/EKS Access
...
6. [ ] Check container logs and manifest for known source repository images, review code changes on GitHub if applicable
7. [ ] Only use kubectl if CloudWatch data is missing or need current state
