...
```bash
kubectl apply -f manifests/01-ebs-storageclass.yaml
```
...
```bash
kubectl apply -f manifests/02-gpu-nodepool.yaml
```
...
bash manifests/03-bifrost-install.sh
```
...
```bash
kubectl apply -f manifests/04-bifrost-config-patch.yaml
...
```bash
kubectl apply -f manifests/05-vllm-deployment.yaml
```
